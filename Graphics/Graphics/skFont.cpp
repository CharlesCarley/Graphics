...
const SKint32      Spacer    = 7;
const skFont::Char NullChar  = {0.f, 0.f, 0.f, 0.f};
...
SKint32 skFont::getAverageWidth(void)
{
    ...
    m_opts.average = SKint32(SKscalar(av + 0.5));
    return m_opts.average;
}
...
{
    skFileStream fs(path, skStream::READ);
    if (!fs.isOpen())
        return false;
    ...
    bool res = loadTTF(data, len, size, dpi);
    delete[] data;
    return res;
}
...
bool skFont::loadTTF(const void* mem, SKuint32 len, SKuint32 size, SKuint32 dpi)
{
    ...
    m_image      = m_ctx->createInternalImage(m_width, m_height, SK_ALPHA);  // Convert back to alpha only
    skImage* ima = m_image->getInternalImage();

    m_chars = (Char*)skCalloc(sizeof(Char), CharTotal);
    ...
    for (i = CharStart; i < CharEnd; ++i)
    {
        ...
        ch.h = (SKscalar)m_yMax;
        ch.o = (SKscalar)skMax<SKint32>(0, m_yMax - yBearing);
